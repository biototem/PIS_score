try:
    ...
    import os
    import sys
    ASAP_bin_path = os.path.join(os.getcwd(), '/opt/ASAP/bin/')  #这里是ASAP的路径，linux下相对应修改一下成为自己的即可
...
class Writer(mir.MultiResolutionImage):
    ...
    def __enter__(self):
        ...
        try:
            ...
            self._writer.setCompression(2)
            self._writer.setDataType(1)
            self._writer.setInterpolation(0)
            self._writer.setColorType(1)
