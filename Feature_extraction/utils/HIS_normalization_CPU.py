...
class his_normalization_pytorch_cpu(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.stain_color_map_hematoxylin =  torch.tensor([0.65, 0.70, 0.29],requires_grad=False)
    def forward(self,img,W_target):
        try:
            image = deconvolution_based_normalization1(torch.as_tensor(img), W_target,self.stain_color_map_hematoxylin).permute(0, 2, 3, 1)[0, :, :, :].numpy()
            return image
        except:
            return img
